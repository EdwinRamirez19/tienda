$(document).ready(function (params) {
    ...
    $('#pacientes-modal').modal({ dismissible: false });
    $('#pacientes-redactar').modal({ dismissible: false });
let paciente_id = ''
let study_id = ''
        ...
        if (rol === "Transcriptora") {
            ...
            return `
                        <div align="center" class="row"> 
                            <a class="redactar  btn-primary btn modal-trigger" data-value="${row.paciente_id}" data-backdrop="static" data-keyboard="false" data-study_id="${row.study_id}" href="#pacientes-redactar">Redactar</a>
                        </div>
                `
        }else{
...
$("body").on('click', '.redactar', function () {
    ...
    $("#guardar_redactar").prop("disabled", false);
...
// codigo para transcripcion de informes 

$("body").on('click', '.transcribir', function () {
    ...
    $("#tipo_patron").on('change', function (params) {
        let tipo_patron = $(this).val()

        const patrones = patronesjson.filter(d => d.tipo_patron_id == tipo_patron);
        const tipoPatron = tipoPatronesjson.filter(elemet => elemet.id == tipo_patron)
        console.table(tipoPatron)
        $("#firma").val(tipoPatron[0].firma)
        $("#medico").val(tipoPatron[0].nombres + ' ' + tipoPatron[0].apellidos)
...
$("#guardar_redactar").on('click', function (e) {
...
$("#guardar_transcribir").on('click', function (e) {

    if ($("#descripcion").val() === "") {

        toastr.success('Ingrese una Descripcion')
    } else {
        $(this).prop("disabled", true);
        $.ajaxSetup({
            headers: {
        ...
        enviar_informe = $.ajax({
            ...
            data: {
                ...
                fecha_entregado: "Null",
                tipo_estudio: $("#tipo_estudio").val(),
                regularidad_estudio: $("#regularidad_estudio").val(),
                estudio_id: $("#estudio_id").val(),
...
$("#cancelar").on('click', function (params) {
    ...
    $('#pacientes-redactar').modal('close');
